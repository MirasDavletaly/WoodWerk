...
echo   WOODWERK
echo   ------------------------------------------
echo   Сайт:        http://127.0.0.1:8090/
echo   Админка:     http://127.0.0.1:8090/admin
echo.
echo   Это окно закрывать нельзя - в нём работает сервер.
echo   Логин admin, пароль будет напечатан ниже при первом запуске.
echo   Чтобы остановить, нажмите Ctrl+C или закройте окно.
echo   ------------------------------------------
echo.

rem Браузер открываем с задержкой, чтобы сервер успел подняться.
start "" /b cmd /c "ping -n 3 127.0.0.1 >nul & start "" http://127.0.0.1:8090/admin"
